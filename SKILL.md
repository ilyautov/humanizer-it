---
name: humanizer-it
description: "Skill for humanizing Italian-language text. Removes signs of AI generation and brings the text back to life. Use ALWAYS when the user asks to: humanize Italian text, remove AI/neural-network traces, make text sound natural/alive, rewrite it like a human, 'umanizzare', remove burocratese, remove fluff, make it less formal. Also use when the user pastes Italian text and says things like 'riscrivi', 'rendilo migliore', 'sembra un robot', 'troppo artificiale'. Works ONLY with Italian. For Russian use humanizer-ru; for English use the original humanizer. Do NOT use for: translation, writing from scratch, grammar-only, code."
license: MIT
---
